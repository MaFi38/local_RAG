...
from services.langchain_utils import get_streaming_rag_chain
...
def chat(query_input: QueryInput):
    ...
    rag_chain = get_streaming_rag_chain(query_input.model.value)
...
def upload_and_index_document(user_id: int, file: UploadFile = File(...)):
    ...
    try:
        ...
        if file_extension in ['.jpeg', '.jpg', '.png']:
            ...
            extracted_data = process_image(temp_file_path, user_id)
...
def process_image(image_path: str, file_id: int) -> Dict:
    try:
        ...
        for i in range(len(data['text'])):
            if int(data['conf'][i]) > 0:  # Filter out low confidence results
                ...
                if text.strip():  # Only process non-empty text
                    ...
                    insert_text_position(
                        document_id=file_id,
                        page_number=1,  # Single page for images
                        text=text,
                        bbox=bbox,
...
async def stream_chat_response(rag_chain, question: str, chat_history: list) -> AsyncGenerator[str, None]:
    response = ""
...
async def stream_chat(query_input: QueryInput):
    ...
    return StreamingResponse(
        stream_chat_response(rag_chain, query_input.question, chat_history),
